...
The categorical chart palette is an 8-hue scale (`chart-1..8`) with light and dark variants defined in `globals.css`, so a theme switch cascades through every chart automatically. Its hues are evenly spaced around the wheel and **deliberately disjoint from the status and accent tokens** (TBD-429): a spending category painted in the over-budget red, the settled green, the warning amber or the brass accent would read as a status it does not have (*status-is-data*). Green and red keep their meaning for money direction and state; categories get their own set. *The One Brass Rule* governs chrome (primary CTAs, the active sidebar/list item, the focus ring) and the palette has no brass slot, so it needs no carve-out. Status states in charts use the `chartColor.*` semantic tokens (`chartColor.over` = `danger`), never a positional chart-N index.

- **chart-1** = Raspberry, **chart-2** = Leaf, **chart-3** = Orchid, **chart-4** = Teal, **chart-5** = Mustard, **chart-6** = Cerulean, **chart-7** = Terracotta, **chart-8** = Indigo. The slot order maximises the separation of adjacent slots: charts with few series use slots 1..N, and the stacked-bar fold uses 1..7 plus "Other".
- **The palette invariant**, in both themes: every slot is >= 3:1 against `surface`; every pair, including "Other" (`border-strong`), is >= 5 CIEDE2000 apart under normal vision and simulated protan, deutan and tritan dichromacy under both the Viénot 1999 and Brettel 1997 models; every slot is >= 12 CIEDE2000 from `success`, `danger`, `warning` and `accent`; a slot's light and dark variants stay within 20° of OKLCH hue. `frontend/tests/lib/chart-palette-invariant.test.ts` measures the tokens in `globals.css` and enforces it, so a palette edit that breaks any of these fails the suite.
...
**The Notice Register.** A widget states a fact its chart cannot draw through one shared notice surface, never a per-feature treatment. **Notice severity is a property of the *(condition, widget)* pair, never of the condition alone.** A notice is **loud** when a number the widget renders was computed from an incomplete set — the reader would act on a figure that is wrong, not merely partial. It is **quiet** when every rendered value is itself correct and only the set is short. The same condition takes both tones: row truncation is quiet on a KPI, line, area or sparkline, and on a **single-dimension** bar or stacked bar, where each mark is its own complete group; it is loud on a pie or a table, where a displayed "total" sums only what came back, and on a **two-dimension** bar or stacked bar, where a returned row is a (primary, secondary) pair so every bar height is itself a partial sum. **The pair, not the widget type:** one component can take both tones depending on how it was configured. **A condition that affects every widget identically is a page notice, never a widget notice.** Scope decides placement; the pair decides tone. Tone is never colour alone — the glyph shape and the sentence both change with it, and the sentence changes again with whether the widget can withhold the bad number (a pie hides its total; a bar cannot hide its bars). **The sentence must also name which end was dropped, and that end is REPORTED, never inferred.** "Truncated" is not one fact: a ranking drops its lowest-ranked tail, a reversed ranking drops the highest, a chronological order drops the oldest or the newest depending on its direction. Telling a reader the newest data was cut when the oldest was is worse than saying nothing. **When the server reports no end, the notice names none** — ordering by a name is alphabetical and has no reader-facing end, so the honest sentence is unqualified. A default that picks an end to look confident is the defect this rule exists to prevent.
...
- **Don't** theme-switch a brand surface. Brand surfaces stay locked navy/brass; chart series are data, and their palette (disjoint from status and accent) is documented in the Data Visualization section.
