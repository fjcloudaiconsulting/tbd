...
describe("DashboardPage — Recent Transactions sort-header tap targets (WCAG 2.5.8)", () => {
  ...
  it("Date, Description, Amount sort buttons carry min-h-[32px]", async () => {
    ...
    await waitFor(() => {
      ...
      // two-stage fetch chain (refs + period, then transactions), which
      // measured 1.7-1.9s on a loaded worker, past the 1000ms default.
    }, { timeout: 4000 });
    ...
    // TBD-288: the first render alone can take ~2s under load, so the test
    // budget must cover it plus the 4000ms settle above.
  }, 10_000);
});
