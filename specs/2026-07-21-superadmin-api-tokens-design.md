...
8. ~~**Direct `api_token_id` audit attribution.**~~ **DONE — TBD-188**, see `specs/2026-08-07-audit-api-token-attribution.md`. v1 attributed PAT-authed actions via `request_id`↔structlog correlation (§8/§11); `audit_events.api_token_id` now gives one-hop separability without a log join. ⚠ The mechanism this line originally prescribed — *"shared helper reading `request.state`"* — was **not** implemented and must not be revived: `request.state.api_token_id` had zero readers and has been deleted, and 22 of the 108 audit call sites are service functions with no `Request` in scope at all. The column is resolved from the request-scoped **structlog contextvar** inside `audit_service._build_audit_event`, so every call site is attributed by construction.
