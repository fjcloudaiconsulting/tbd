...
**R5 (was N1) — UI display scope. UPDATED (product decision, locked 2026-06-01): GROUPED display is IN SCOPE.** The owner's framing is "the forecast is always for the master," so the list AND chart must visually group/roll up subcategory items under their master:
- List: sub items render under a master header row whose planned/actual/variance totals are the SUM of that master's sub items (`renderMasterHeader` in `ForecastPlansClient.tsx`). Master-level items (master mode) render as a single self-named row, unchanged. Implemented via `groupByMaster` producing `MasterGroup[]`, consumed by `ItemSection`.
- Chart: bars plot at the master level (`chartData` aggregates `expenseGroups`, one bar per master = sum of its subs).
Totals stay correct (already summed across items). Edit/remove still operate on the individual sub rows.
