...
class Transaction(Base):
    ...
    # L3.2 Wave 2B: reconciliation state machine + import-batch link.
    # NULL ``import_batch_id`` means "not from an import" (manual entry,
    # synthetic transfer leg, recurring expansion, etc.). The
    # ``reconciliation_state`` column defaults to ``ACCEPTED`` for ALL
    # rows -- imported or not -- per spec §3.2.1.
    ...
    reconciliation_state: Mapped[str] = mapped_column(
        Enum(
            ...
            "accepted",
            "rejected",
            name="transaction_reconciliation_state",
        ),
        nullable=False,
        default="accepted",
        server_default="accepted",
    )
    # OFX cross-batch dedup: bank-assigned unique ID per OFX spec §11.4.4.
    # NULL for CSV rows and any OFX row whose ``<FITID>`` was missing.
    fitid: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
