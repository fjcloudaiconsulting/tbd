...
async def test_lifespan_starts_and_stops_scheduler(monkeypatch):
    started = {"n": 0, "max_orgs": None}
    async def _fake_loop(stop_event, *, tick_seconds, lock_ttl, max_orgs=None):
        started["n"] += 1
        started["max_orgs"] = max_orgs
    ...
    monkeypatch.setattr(app_main.app_settings, "scheduler_max_orgs_per_tick", 25)
    ...
    async with app_main.lifespan(app_main.app):
        ...
        assert started["n"] == 1
        assert started["max_orgs"] == 25  # rollout-guard cap threaded from settings
