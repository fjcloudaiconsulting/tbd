...
| `OFX_PARSE_MAX_PER_ORG` | no | `2` | `.env` (optional) | unset | inherit default | no | Per-org ceiling on concurrent OFX parses, so one org cannot occupy every global slot and starve others. Over-cap → HTTP 429 immediately (non-blocking). Set `0` to disable the per-org cap (global cap still applies). | Default 2. |
